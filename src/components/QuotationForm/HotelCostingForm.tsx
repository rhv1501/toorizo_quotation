...
const HotelCostingForm: React.FC = () => {
  ...
  function getHotelsFor(location: string, packageType: string) {
    ...
    return hotels.filter(
      (h) =>
        ...
        h.packageType === targetPackage
    );
  }

  // Helper to calculate average hotel cost for a package type and specific location
  const getLocationAverageHotelCost = React.useCallback(
    (location: string, packageType: string, nights: number): number => {
      const packageKey = (packageType || "").toLowerCase() as
        | "standard"
        | "comfort"
        | "luxury";
      ...
      const total = hotelsForLoc.reduce(
        (sum, hotel) => sum + hotel.tacSeason,
        0
      );
  ...
  const getTotalAverageHotelCost = React.useCallback(
    (packageType: string): number => {
      ...
      selectedLocations.forEach((location) => {
        ...
        const locationNights =
          hotelCostingDetails?.locationDays?.[location] ||
          Math.floor(totalNights / selectedLocations.length);
        const locationCost = getLocationAverageHotelCost(
          location,
          packageType,
          locationNights
        );
    ...
    [
      selectedLocations,
      hotelCostingDetails?.skippedLocations,
      hotelCostingDetails?.locationDays,
      totalNights,
      getLocationAverageHotelCost,
    ]
  );
  ...
  React.useEffect(() => {
    if (
      ...
      packageTypes.forEach(({ key, packageType }) => {
        ...
        setValue(
          `hotelCostingDetails.${key}.packageType` as any,
          `${key.charAt(0).toUpperCase() + key.slice(1)} Package`
        );
      });
    }
  }, [
  ...
  React.useEffect(() => {
    if (customerRequirements?.type !== "travel_only") {
      ...
      const standardFinalCost =
        standardCostWithMargin + (standardExtraCost || 0);
  ...
  const handleNightDistribution = (location: string, nights: number) => {
    ...
    const otherLocationsTotal = Object.entries(currentLocationNights)
      ...
      .reduce(
        (sum, [, locNights]) =>
          sum + (typeof locNights === "number" ? locNights : 0),
        0
      );
    ...
    if (otherLocationsTotal + nights <= totalNights) {
      ...
      if (!hotelCostingDetails?.locationDetails?.[location]) {
        ...
        setValue(
          `hotelCostingDetails.locationDetails.${location}.days`,
          nights
        );
      }
  ...
  const handleLocationDateChange = (
    ...
    if (!hotelCostingDetails?.locationDetails?.[location]) {
      ...
      setValue(
        `hotelCostingDetails.locationDetails.${location}.${field}`,
        date
      );
    }
  };
    ...
    return (
      <div className="space-y-6">
        <h3 className="text-lg font-medium text-gray-900">
          Hotel Costing Details
        </h3>
        <div className="p-4 border rounded-md bg-yellow-50">
          <p className="text-yellow-800">
            Hotel costing is disabled because "Travel Only" is selected in
      ...
      <div className="space-y-6">
        ...
        <div className="p-4 border rounded-md bg-gray-50">
          <p className="text-gray-600">
            Please select locations in the Itinerary section first to configure
            hotel costing.
          </p>
        </div>
      </div>
      ...
      {isMultipleLocations && (
        <div className="p-4 border rounded-md bg-blue-50">
          <h4 className="text-sm font-medium text-blue-900 mb-4">
            Location Details & Night Distribution
          </h4>
          <div className="space-y-4">
            {selectedLocations.map((location) => {
              ...
              const locationDetails =
                hotelCostingDetails?.locationDetails?.[location];
              const isSkipped =
                hotelCostingDetails?.skippedLocations?.[location] || false;
                ...
                >
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <div className="flex items-center space-x-2">
                      <input
                        ...
                        checked={isSkipped}
                        onChange={(e) =>
                          handleLocationSkip(location, e.target.checked)
                        }
                      />
                      <label
                  ...
                  {isSkipped && (
                    <div className="text-center py-4 text-gray-500">
                      <p className="text-sm">
                        Hotel booking is skipped for this location
                      </p>
                    </div>
                  )}
                </div>
          ...
          <p className="mt-4 text-sm text-blue-700">
            Total allocated:{" "}
            {Object.values(hotelCostingDetails?.locationDays || {}).reduce(
              (sum: number, nights) =>
                sum + (typeof nights === "number" ? nights : 0),
              0
            )}{" "}
            / {totalNights} nights
          </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <FormField label="Check-in Date">
              <input
                ...
                className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-teal-500 focus:ring-teal-500 sm:text-sm"
                value={
                  hotelCostingDetails?.locationDetails?.[selectedLocations[0]]
                    ?.checkInDate
                    ? format(
            ...
            {isMultipleLocations && baseCost > 0 && user?.role === "admin" && (
              <div className="mt-4 p-3 border rounded-md bg-white">
                ...
                <div className="text-sm text-gray-700 space-y-1">
                  {selectedLocations.map((location) => {
                    const isSkipped =
                      hotelCostingDetails?.skippedLocations?.[location] ||
                      false;
                    if (isSkipped) {
                      return (
              ...
              <div className="mt-4 p-3 border rounded-md bg-white">
                ...
                <div className="text-sm text-gray-700 space-y-1">
                  ...
                  <p className="font-semibold">
                    Final Hotel Cost: ₹{finalCost}
                  </p>
                  <p className="text-xs text-gray-500">
                    Calculation: {Math.round(baseCost * 1.15)} +{" "}
                    {extraCost || 0} = {finalCost}
                  </p>
                </div>
              </div>
            )}
